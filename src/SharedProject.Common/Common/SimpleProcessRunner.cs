...
namespace SimpleTrace.Common
{
    public class SimpleProcessInfo
    {
        ...
        {
            return string.Format("ProcessInfo => [{0}, {1}, {2}]", ProcessName, ExePath, ExeArgs);
        }
    ...
    {
        public SimpleProcess(SimpleProcessInfo info)
        {
            Info = info;
        }
    ...
    public class SimpleProcessRunner
    {
        ...
        public Task<MessageResult> TryStart(bool failThrows = false)
        {
            ...
            {
                Log.LogEx(ex, "process start ex! " + Process.Info);
                if (failThrows)
                {
                    throw;
                }
        ...
        public Task<MessageResult> TryStop(bool failThrows = false)
        {
            ...
            {
                Log.LogEx(ex, "process stop ex! " + Process.Info);
                if (failThrows)
                {
                    throw;
                }
